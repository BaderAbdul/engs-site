...
export default function Home() {
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 space-y-20">
      ...
      <section className="bg-qec-brown rounded-[3rem] p-10 text-white shadow-xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
          <div>
            <h2 className="text-3xl font-bold mb-2">المستودع الرقمي</h2>
            <p className="text-white/80">تتبع توفر القطع والمعدات في المعمل</p>
          </div>
          <Link href="/inventory" className="bg-white text-qec-brown px-6 py-3 rounded-xl font-bold hover:bg-slate-100 transition shadow-md">
            طلب استعارة قطعة
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={item.name} className="bg-white/10 border border-white/20 p-5 rounded-2xl backdrop-blur-sm">
              <Box className="text-qec-teal mb-3" size={20} />
              <h4 className="font-bold text-sm">{item.name}</h4>
              <p className="text-2xl font-black mt-2 text-qec-teal">{item.count}</p>
              <p className="text-xs text-white/70 mt-1 font-medium">متوفرة حالياً</p>
            </div>
          ))}
        </div>
      </section>

      {/* 4. المرجع السريع (Quick Pinouts) - تم التحديث بألوان QEC */}
      <section className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        <div className="bg-qec-blue h-80 rounded-[2.5rem] relative overflow-hidden shadow-2xl shadow-blue-100">
