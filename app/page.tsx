  ...
  return (
    <div className="relative min-h-screen bg-[#020617] text-white overflow-x-hidden font-sans selection:bg-qec-blue/30">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 pt-8 pb-20 relative z-10 space-y-16 md:space-y-32 text-right">
        ...
        <section className="flex flex-col items-center text-center pt-10 md:pt-20">
          ...
          >
            <div className="p-2 bg-qec-blue/10 rounded-xl border border-qec-blue/20">
              <Cpu size={28} strokeWidth={1.5} className="text-qec-blue" />
            </div>
          ...
          {/* أزرار الإجراء العامة للمنصة (فك الارتباط المربك) */}
          <div className="flex flex-wrap justify-center gap-3 mt-8">
             <Link href="/signup" className="bg-qec-blue hover:bg-blue-600 text-white px-8 py-3 rounded-xl font-black text-sm transition-all shadow-md flex items-center gap-2 active:scale-95">
          ...
          {/* 2. Bento Stats: توحيد سماكة الأيقونات 1.5 */}
        ...
        {/* 3. الحدث الأهم: الدورة القادمة (مستقلة مع زر الحجز الخاص بها) */}
          ...
          <div className="relative z-10 flex flex-col lg:flex-row-reverse gap-10 items-center">
            ...
            <div className="w-full lg:w-7/12 text-right space-y-6">
              ...
              <h1 className="text-3xl md:text-5xl font-black leading-tight text-white tracking-tight">
                طور <span className="text-qec-blue">مهاراتك العملية</span>
              </h1>
            ...
            {/* بطاقة الحدث الأبرز - لغة عربية بحتة وزر حجز داخلي */}
            <div className="w-full lg:w-5/12 bg-[#020617]/50 rounded-[2rem] p-8 border border-white/10 shadow-[0_0_30px_rgba(59,130,246,0.1)] space-y-6 group-hover:shadow-[0_0_50px_rgba(59,130,246,0.15)] transition-shadow">
              <div className="flex justify-between items-center border-b border-white/5 pb-4">
                 ...
                 <Cpu className="text-qec-blue opacity-50" size={24} strokeWidth={1.5} />
              </div>
              <h3 className="text-xl md:text-2xl font-black group-hover:text-qec-blue transition-colors">{mainEvent.title}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-slate-400 font-bold border-y border-white/5 py-4">
                  <div className="flex items-center justify-end gap-2 text-right"><span>29 أبريل | 9:00 ص</span><Calendar size={14} className="text-qec-blue"/></div>
                  <div className="flex items-center justify-end gap-2 text-right"><span>قاعة وادي التقنية</span><MapPin size={14} className="text-qec-blue"/></div>
              </div>
              <div className="space-y-4">
                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full w-4/5 bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]"></div>
                </div>
                <p className="text-[10px] text-amber-500 font-bold text-center uppercase tracking-widest italic animate-pulse">⏳ تبقى 5 مقاعد فقط</p>
                {/* زر الإجراء المرتبط بالدورة (CTA Logic) */}
                <Link href={`/events/${mainEvent.id}`} className="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white py-3 rounded-xl font-black text-sm transition-all flex items-center justify-center gap-2 mt-2">
        ...
        {/* 4. لوحة التميز وتفاعل الأقسام: إضافة الأفاتار للوحة التميز */}
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
              ...
              {displayStudents.map((s, idx) => (
                <div key={s.id} className="flex items-center justify-between p-6 border-b border-white/5 last:border-0 hover:bg-white/[0.03] transition-colors">
                  ...
                  <div className="flex items-center gap-4 text-right flex-row-reverse">
                    ...
                    {/* إضافة الأفاتار البصري هنا لكسر الجمود */}
          ...
          <div className="space-y-8 text-right px-2">
            <h2 className="text-xl font-black flex items-center gap-2 justify-end text-slate-200">
              تفاعل الأقسام <Layout size={20} strokeWidth={1.5} className="text-qec-teal" />
            </h2>
        ...
        {/* 5. معرض المبتكرين (التعريب الكامل) */}
